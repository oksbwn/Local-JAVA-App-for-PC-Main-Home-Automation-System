...
public class ReadMailSample {  
	...
	public void readMails(int type) {  
		...
		session = Session.getInstance(properties,new javax.mail.Authenticator() 
		   ...
		   try {store = session.getStore("imaps");       
		        ...
		        Message messages[] = inbox.search(new FlagTerm(new Flags(Flag.SEEN),false));; //SEEN
		         // System.out.println("Number of mails = " + messages.length);    
		      //  final Voice voiceKevin16 = new Voice("kevin16");
		        if (messages.length==0 && type==0){
		        	new  Voice("Sir you have no E-mail..");
		        	new popMe("No new mail","Gmail" , "email", 6,125);
			       	return;
		        	   }
		        else if( type==0){
				   new Voice("You have got "+messages.length+"E-mails.");
		          new popMe("No of mails : "+messages.length,"Gmail" , "email", 6,125);
		        }
		          int n = 0;
		          if(messages.length>0 && type==0)
		       { 
		       ...
		       if (n==0 && messages.length>0){  
                ...
                //.showMessageDialog(null,);
                   int nO = JOptionPane.showOptionDialog(null,mailData+mailData1,"What Do You Want Me todo",
                    ...
                    if(nO==1){processMessageBody(message);  }
                    if(nO==2){inbox.setFlags(new Message[]{message},new Flags(Flags.Flag.DELETED),true);
