...
import com.oksbwn.emails.ReadMailFromGMAIl;
...
public class PopMe extends Thread{
	...
	public PopMe(String header, String message, String image,boolean isMail){
		...
		this.isMail=isMail;
		try {
			if(InetAddress.getByAddress(new byte[]{(byte) 192,(byte) 168,0,100}).isReachable(1000))
				new SendData().sendPost("http://192.168.0.1/smart_home/API/utilities_all/xbmc/showMessage.php", new String[]{"FROM","MSG"}, new String[]{header,message});
